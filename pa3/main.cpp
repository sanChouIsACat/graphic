...
Eigen::Matrix4f get_model_matrix(float angle) {
  return transformation::generateArbitraryRotateMatrix(
      POINT_EGDE_3D{0, 1, 0, 0}, POINT_EGDE_3D{0, 0, 0, 1}, angle);
}
...
int main(int argc, const char **argv) {
  ...
  for (auto mesh : Loader.LoadedMeshes) {
    for (int i = 0; i < mesh.Vertices.size(); i += 3) {
      ...
      for (int j = 0; j < 3; j++) {
        t->setVertex(j,
                     POINT_EGDE_3D(mesh.Vertices[i + j].Position.X,
                                   mesh.Vertices[i + j].Position.Y,
                                   0 - mesh.Vertices[i + j].Position.Z, 1.0));
        t->setNormal(j, POINT_EGDE_3D(mesh.Vertices[i + j].Normal.X,
                                      mesh.Vertices[i + j].Normal.Y,
                                      0 - mesh.Vertices[i + j].Normal.Z, 1.0));
        t->setTexCoord(
            j, POINT_EGDE_2D(mesh.Vertices[i + j].TextureCoordinate.X,
                             mesh.Vertices[i + j].TextureCoordinate.Y, 1.0));
  ...
  g_sharder::EnvPayload env{light_defition, Vector3f{10, 10, 10}};
