...
{
    compute_event_loop.add_event((std::function<void()>)std::bind(&LineDebugger::compute_mvp, this), "compute");
...
{
    interpolation::tanx_line_draw(from3dTo2d(tri.v[0]), from3dTo2d(tri.v[1]), set_pixel_f);
    interpolation::tanx_line_draw(from3dTo2d(tri.v[1]), from3dTo2d(tri.v[2]), set_pixel_f);
    interpolation::tanx_line_draw(from3dTo2d(tri.v[2]), from3dTo2d(tri.v[0]), set_pixel_f);
}

void LineDebugger::draw_line(const Line& line)
{
    interpolation::tanx_line_draw(from3dTo2d(line.a), from3dTo2d(line.b), set_pixel_f);
}
...
void LineDebugger::draw_f()
{
    ...
    while (it != mvp_primitives.end()) {
        ...
        // just a debugger, may use other design mode that couple raster algo with concreate primitive types
...
{
    GComponent::GameWindow<CameraStateMachine>::DRAR_FUNCTION f = std::bind(&LineDebugger::draw_f, this);
    gameWindow.set_draw_functions(GComponent::GameWindow<CameraStateMachine>::DRAR_FUNCTIONS{ f });
    compute_mvp();
