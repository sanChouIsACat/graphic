...
GComponent::pos_buf_id GComponent::rasterizer::load_positions(
...
GComponent::ind_buf_id GComponent::rasterizer::load_indices(
...
GComponent::col_buf_id
GComponent::rasterizer::load_colors(const std::vector<Eigen::Vector3f> &cols) {
  auto id = get_next_id();
...
GComponent::col_buf_id GComponent::rasterizer::load_normals(
...
void GComponent::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
  ...
  for (const auto &t : TriangleList) {
    ...
    std::array<POINT_EGDE_3D, 3> viewspace_pos = {mv * t->v[0], mv * t->v[1],
                                                  mv * t->v[2]};
    ...
    POINT_EGDE_3D n[] = {inv_trans * t->normal[0], inv_trans * t->normal[1],
...
void GComponent::rasterizer::rasterize_triangle(
    const Triangle &t, const std::array<Eigen::Vector4f, 3> &view_pos) {
  ...
  for (int i = min_x; i <= std::ceil(max_x); i++) {
    for (int j = min_y; j < std::ceil(max_y); j++) {
      ...
      auto [a, b, c, depth, view_coor_normal, view_coor_point, text_coors] =
          GAlgo::BarycentricProperties(i, j, t.v, depths, t.normal, view_pos,
                                       t.tex_coords);
      ...
      g_sharder::FragmentShaderPayload shader_payload(
          pixel, view_coor_normal, view_coor_point, text_coors);
      RGB color = shader.shade_pixel(shader_payload);
      // G_LOGGER_INFO("x[%d] y[%d] ORIGIN RGB[%s]", i, j,
      // EigenStructToString(pixel.transpose()).c_str()); G_LOGGER_INFO("x[%d]
      // y[%d] RGB[%s]", i, j, EigenStructToString(color.transpose()).c_str());
      set_pixel(Vector2i(i, j), color);
    }
  }
}

// void GComponent::rasterizer::rasterize_triangle(const Triangle& t, const
...
//                //float w_reciprocal = 1.0 / (alpha / v[0].w() + beta /
//                v[1].w() + gamma / v[2].w()); float z_interpolated = alpha *
//                v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma *
//                v[2].z() / v[2].w();
...
void GComponent::rasterizer::set_line_draw_algo(
...
void GComponent::rasterizer::set_model(const Eigen::Matrix4f &m) { model = m; }

void GComponent::rasterizer::set_view(const Eigen::Matrix4f &v) { view = v; }
...
void GComponent::rasterizer::set_pixel(const Vector2i &point,
