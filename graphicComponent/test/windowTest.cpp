...
template<typename U>
std::thread startLoop(GameWindow<U>& gameWindow) {
...
struct LButtonMoving : public boost::statechart::state<LButtonMoving, Machine> {
    ...
    boost::statechart::result react(const MouseKeyboardEvent& event) {
        if (event.id == cv::EVENT_LBUTTONUP) {
            ...
            std::function<void(const Eigen::Vector3f&, const Eigen::Vector3f&)> set_pixel_f = 
                std::bind(&FrameBuffer::set_pixel,
                    &buffer,
                    std::placeholders::_1,
                    std::placeholders::_2
                );
...
TEST(GameWindowTest, eventTest) {
    ...
    GameWindow<Machine> gameWindow(700, 700, "test",machine);
    auto thread = startLoop(gameWindow);
    gameWindow.inject_gui_event(MouseKeyboardEvent{ cv::EVENT_LBUTTONDOWN,300,300 });
    gameWindow.inject_gui_event(MouseKeyboardEvent{ cv::EVENT_MOUSEMOVE,300,301 });
    gameWindow.inject_gui_event(MouseKeyboardEvent{ cv::EVENT_LBUTTONUP,300,350 });
