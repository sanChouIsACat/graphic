...
void Triangle::setNormal(int ind, POINT_EGDE_3D n){
    normal[ind] = n;
}
...
void Triangle::setTexCoord(int ind, POINT_EGDE_2D uv) {
    tex_coords[ind] = uv;
}
...
std::array<POINT_EGDE_3D, 3> Triangle::toVector4() const
{
    ...
    std::transform(std::begin(v), std::end(v), res.begin(), [](auto& vec) { return POINT_EGDE_3D(vec.x(), vec.y(), vec.z(), 1.f); });
    return res;
}
