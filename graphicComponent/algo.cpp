...
Eigen::Vector3f GAlgo::computeLightPlaneInterSection(const Light& light, const Plane& plane) {
...
std::tuple<bool, Eigen::Matrix<float, 3, 2>> GAlgo::computeLightAABBInterSection(const Light& light, const AABB& aabb)
{
	...
	// 底面
	planes[1] = {
		aabb.bottom_2 - aabb.bottom_1,
		aabb.bottom_4 - aabb.bottom_1,
		aabb.bottom_1
	};

	// 前面
	planes[2] = {
		aabb.top_1 - aabb.bottom_1,
		aabb.bottom_2 - aabb.bottom_1,
		aabb.bottom_1
	};
	...
	// 后面
	planes[3] = {
		aabb.top_4 - aabb.bottom_4,
		aabb.bottom_3 - aabb.bottom_4,
		aabb.bottom_4
	};
	...
	// 右面
	planes[5] = {
		aabb.top_2 - aabb.bottom_2,
		aabb.bottom_3 - aabb.bottom_2,
		aabb.bottom_2
	};
	...
	for (int i = 0; i < arr_len && ans_num < 2; i++)
	{
		...
		if (slove[0] > 0 && slove[1] <=1 && slove[2] <=1) res.col(ans_num++) = slove;
	}
	if (ans_num < 2) {
		return {false, RES_T{}};
	}
