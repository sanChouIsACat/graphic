...
LinearCameraControl::LinearCameraControl(const GTypes::AABB &aabb,
    ...
    : aabb(aabb), screen_width(screen_width), screen_height(screen_height),
      ratio_x(ratio_x), ratio_z(ratio_z), camera(camera), last_time_x_press(0),
      last_time_z_press(0), press_threshlod(20), last_time_mouse_coordiante(){};
...
void LinearCameraControl::computeMoveStep(long long &ratio,
                                          ...
                                          long long &timestamp, int &screen_len,
                                          int offset) {
  long long interval = current_timestamp - last_time_x_press;
  float step_len =
      screen_len * (interval > press_threshlod ? 20 : interval) / ratio;
