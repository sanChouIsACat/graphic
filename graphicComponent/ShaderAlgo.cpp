...
Eigen::Vector3f g_sharder::phone_sharder_algo(const FragmentShaderPayload& payload, const EnvPayload& env)
{
	...
	for (const auto& light : env.lights) {
		...
		// add one to avoid infinite value at nearest poztion(let light point be distance 1)
...
Eigen::Vector3f g_sharder::normal_fragment_shader(const FragmentShaderPayload& payload)
{
