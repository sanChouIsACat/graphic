...
{
	camera_point = transformation::generateArbitraryRotateMatrix({ 1,0,0,0 }, { 0,0,0,1 }, angle) * camera_point;
}

void GComponent::Camera::rotateByY(float angle)
{
	camera_point = transformation::generateArbitraryRotateMatrix({ 0,1,0,0 }, { 0,0,0,1 }, angle) * camera_point;
}

void GComponent::Camera::rotateByZ(float angle)
{
	camera_point = transformation::generateArbitraryRotateMatrix({ 0,0,1,0 }, { 0,0,0,1 }, angle) * camera_point;
}

Eigen::MatrixX4f GComponent::Camera::getAndUpdateViewTransformMatrix()
{
	//G_LOGGER_INFO("camera move to %s", EigenStructToString(old_view_transform).c_str());
