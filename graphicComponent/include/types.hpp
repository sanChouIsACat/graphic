...
struct AABB {
  ...
  AABB(const POINT_EGDE_3D &t1, const POINT_EGDE_3D &t2,
       ...
       const POINT_EGDE_3D &b3, const POINT_EGDE_3D &b4)
      : top_1(t1), top_2(t2), top_3(t3), top_4(t4), bottom_1(b1), bottom_2(b2),
        bottom_3(b3), bottom_4(b4) {}
};
...
struct Light {
  ...
  POINT_EGDE_3D dir;
  Light(const RGB &rgb, const POINT_EGDE_3D &position, const POINT_EGDE_3D &dir)
      : rgb(rgb), position(position), dir(dir){};
  Light(const Light &b) {
    this->rgb = b.rgb;
...
struct EnvPayload {
  ...
  EnvPayload(const std::vector<GTypes::Light> &lights, const RGB &env_light,
             float p = 10)
      : lights(lights), env_light(env_light), p(p){};
  EnvPayload(const EnvPayload &b)
      : lights(b.lights), env_light(b.env_light), p(b.p) {}

  EnvPayload(EnvPayload &&b) noexcept
      : lights(std::move(b.lights)), env_light(std::move(b.env_light)), p(b.p) {
  }
};
struct FragmentShaderPayload {
  // assume texture pixel's RGB value is under standard env, which object is
  // exposed to (255,255,255) light at distance 1
  FragmentShaderPayload(const RGB &col, const POINT_EGDE_3D &nor,
                        const POINT_EGDE_3D &view_pos, const POINT_EGDE_2D &tc)
      : color_reflection_coefficient(col), normal(nor), view_pos(view_pos),
        tex_coords(tc) {}

  FragmentShaderPayload(RGB &&col, POINT_EGDE_3D &&nor,
                        POINT_EGDE_3D &&view_pos, POINT_EGDE_2D &&tc)
      : color_reflection_coefficient(col), normal(nor), view_pos(view_pos),
        tex_coords(tc) {}
...
using FragmentShaderAlgo = std::function<Eigen::Vector3f(
    const FragmentShaderPayload &, const EnvPayload &)>;
using VertexShaderAlgo = std::function<Eigen::Vector3f(
    const VertexShaderPayload &, const EnvPayload &)>;
} // namespace g_sharder
