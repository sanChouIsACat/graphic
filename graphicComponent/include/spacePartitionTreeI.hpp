...
namespace GAlgo {
	...
	public:
		...
		using PARITION_ALGO = std::function<std::tuple<std::vector<T>, std::vector<T>>(TreeNodes*)>;
	...
	public:
		virtual SpacePartitionTreeI(int aabb_ele_size,
			PARITION_ALGO partition_f,
			LOCATE_ALGO locate_f) :
			aabb_ele_size(aabb_ele_size),partition_f(partition_f),locate_f(locate_f) {};
