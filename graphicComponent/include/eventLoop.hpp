...
template <typename... Args> using EventFunction = std::function<void(Args...)>;
...
EventFunctionPack packEvent(EventFunction<Args...> event_function,
                            const std::string &event_name, Args... args) {
  return EventFunctionPack{std::bind(event_function, std::placeholders::_1,
...
private:
   ...
   *
   * that is if the event execution time is less than loop_interval, loop will
   * wait a few time. But if the event exection time is more than loop_interval,
...
public:
  ...
  template <typename... Args>
  bool add_event(EventFunction<Args...> event_function,
                 const std::string &event_name, Args... args) {
    mutex.lock();
    if (ring_buffer.full()) {
      mutex.unlock();
    ...
    ring_buffer.push_back(eventLoopInternal::EventFunctionPack{
        std::bind(event_function, std::forward<Args>(args)...), event_name});
    mutex.unlock();
    return true;
  }
  void start() {
  ...
  void stop() { stop_condition = true; }

  bool isStop() { return stop_condition; }
};
} // namespace GComponent
