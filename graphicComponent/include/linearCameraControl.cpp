...
GComponent::LinearCameraControl::LinearCameraControl(
    const GTypes::AABB &aabb, int screen_width, int screen_height,
    float ratio_z, float ratio_x, GComponent::Camera &camera)
    : aabb(aabb), screen_width(screen_width), screen_height(screen_height),
      ratio_z(ratio_z), ratio_x(ratio_x), camera(camera) {}

void GComponent::LinearCameraControl::keyBoardX(int x) {
  float step = computeMoveStep(this->ratio_x, this->last_time_x_press,
                               this->screen_width, x);
  camera.move(transformation::generateTranslateMatrix(step, 0, 0));
}

void GComponent::LinearCameraControl::keyBoardZ(int y) {
  float step = computeMoveStep(this->ratio_z, this->last_time_z_press,
                               this->screen_width, y);
...
float GComponent::LinearCameraControl::computeMoveStep(long long &ratio,
                                                       long long &timestamp,
                                                       int &screen_len,
                                                       int sign) {
