...
public:
  ...
  template <typename T> void set_shader(T &&shader) {
    static_assert(std::is_same_v<g_sharder::Shader, std::decay_t<T>>,
                  "using g_sharder::Shader");
...
private:
  void rasterize_triangle(const GTypes::Triangle &t,
                          const std::array<Eigen::Vector4f, 3> &world_pos);
