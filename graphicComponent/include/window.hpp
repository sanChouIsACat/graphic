...
struct MouseKeyboardEvent
    : public boost::statechart::event<MouseKeyboardEvent> {
  int id;
  int x;
  int y;
  ...
  MouseKeyboardEvent(int event, int x, int y, void *gameWindow = nullptr)
      : id(event), x(x), y(y), gameWindow(gameWindow){};
};
template <typename StateMachine> class GameWindow : public FrameBuffer {
public:
...
public:
  GameWindow(int w, int h, const std::string window_name, StateMachine &m)
      : FrameBuffer(w, h), window_name(window_name), state_machine(m),
        window(w, h, CV_32FC3, frame_buf.data()) {
    cv::cvtColor(window, window, cv::COLOR_RGB2BGR);
  }

  void exitNow() { gui_loop.stop(); }
  ...
  void inject_gui_event(const MouseKeyboardEvent &event) {
    ...
    EventFunction<> process_event = [this, &event]() {
      state_machine.process_event(
          MouseKeyboardEvent{event.id, event.x, event.y, this});
    };
