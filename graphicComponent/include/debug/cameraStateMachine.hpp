...
/*
* unit: pixel.
* The stateMachine only response for set input. Computing will be finished in another code block.
*/
...
struct CameraStateMachine : public boost::statechart::state_machine<CameraStateMachine, State> {
...
struct State : public boost::statechart::state<State, CameraStateMachine> {
    ...
    State(my_context ctx) : boost::statechart::state<State, CameraStateMachine>(ctx) {}
    CameraMachineContext& getCtx() const {
        return outermost_context().ctx;
    }
    boost::statechart::result react(const GComponent::MouseKeyboardEvent& event) {
        ...
        if (event.id == cv::EVENT_MOUSEMOVE) {
            std::function<void()> mouse_f = std::bind(&GComponent::CameraControlI::mouse,
                &ctx.cameraControlI,
                event.x,
                event.y);
            ...
            std::function<void()> keyboard_w = std::bind(&GComponent::CameraControlI::keyBoardZ,
                &ctx.cameraControlI,
                1,
                getCurrentTimeStamp());
            ...
            std::function<void()> keyboard_s = std::bind(&GComponent::CameraControlI::keyBoardZ,
                &ctx.cameraControlI,
                1,
                getCurrentTimeStamp());
            ...
            std::function<void()> keyboard_a = std::bind(&GComponent::CameraControlI::keyBoardZ,
                &ctx.cameraControlI,
                -1,
                getCurrentTimeStamp());
            ...
            std::function<void()> keyboard_d = std::bind(&GComponent::CameraControlI::keyBoardZ,
                &ctx.cameraControlI,
                +1,
                getCurrentTimeStamp());
