...
#include "logger.hpp"
int computeKnotVSize(int n, int p) { return 2 * (p) + n; }
void GAlgo::BSplineCurve::printBuffer(float t) {
  for each (auto &pair in buffer) {
    G_LOGGER_TRACE("[%f]: [%d,%d] is [%f].", t, pair.first.i, pair.first.k,
                   pair.second);
  }
}
void GAlgo::BSplineCurve::drawCurve(
  ...
  last_time_control_points =
      &std::remove_const_t<std::vector<POINT_EGDE_2D>>(control_point);
  std::vector<POINT_EGDE_2D> &operate_v =
      std::remove_const_t<std::vector<POINT_EGDE_2D>>(control_point);
  ...
  if ((int)control_point.size() - p <= 2) {
    ...
    if (&control_point != last_time_control_points) {
      ...
      std::copy(control_point.begin(), control_point.end(),
                new_control_points.begin());
      for (int i = 0; i < new_control_points.size() - control_point.size();
           i++) {
        new_control_points[new_control_points.size() - 1 - i] =
            control_point[control_point.size() - 1];
      }
    }
  ...
  for (float i = 0; i <= 1; i += sample_rate) {
    ...
    // G_LOGGER_TRACE("[%f:set pixel [%d,%d]", i, (int)std::round(x),
    // (int)std::round(y));
    set_pixel_f(std::round(x), std::round(y), color_f(i));
  }
}

void GAlgo::BSplineCurve::setP(int p) { this->p = p; }
...
float GAlgo::BSplineCurve::coxDeBoor(float t, int i, int k) {
  ...
  if (computed_value != buffer.end())
    return (*computed_value).second;
  float ans;
  float first_value = -1;
  float second_value = -1;
  if (k == 0) {
    if (t < knot_vector[i + 1] && t >= knot_vector[i])
      ans = 1;
    else
      ans = 0;
  } else {
  ...
  buffer[key] = ans;
  /*G_LOGGER_TRACE("current computing [%d,%d] is [%f], the first value[%d,%d] is
     [%f], the second value[%d,%d] is [%f]."
