...
//            //G_LOGGER_INFO("event %s triggered for main", event.name.c_str());
//            exit = event.function(main_window);
//            //G_LOGGER_INFO("event %s triggered for replica", event.name.c_str());
//            event.function(replica_window);
//            event_loop.pop();
//        }
//        cv::imshow(replica_window_name, main_window);
//        cv::imshow(main_window_name, replica_window);
...
//}
//
//bool moveCircle(cv::Mat& window, cv::Point old_center, cv::Point new_center, float radius, const cv::Scalar& color) {
...
//int sample_feq = 5;
//
//void g_curves::Curves::mouse_handler(int event, int x, int y, int flags, void* userdata)
//{
//    tick = (tick + 1) % sample_feq;
//    Curves* _this = static_cast<Curves*>(userdata);
//    POINT_EGDE_2D coor{ (float)x,(float)y,1 };
//    auto circleFinder = [&coor, radiu = _this->control_circle_radiu](const POINT_EGDE_2D& a) {
...
//        std::unordered_set<POINT_EGDE_2D>::iterator find_it = std::find_if(_this->control_points.begin(),
...
//        EventFunctionPack packed = packEvent<cv::Point,cv::Point, float, const cv::Scalar&>(moveCircle,
//            "move circle",
...
//            _this->event_loop.push(packEvent<cv::Point, float, const cv::Scalar&>(drawTemporaryControlSeq,
//                "draw circle",
//                cv::Point{ x,y },
//                _this->control_circle_radiu,
//                WHITE)
