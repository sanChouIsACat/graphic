...
int main() {
  ...
  GameWindow<std::any>::DRAR_FUNCTION draw_curve = [&window, &bSplineCurve,
                                                    &control_points, &ctx]() {
    std::shared_lock lock(ctx.vec_mutex);
    bSplineCurve.drawCurve(sample_rate, control_points, generateRainbowColor);
  };

  // To use bezier curve, uncomment these and comment previous BSplineCurve
  // definition
  // BezierCurve bezierCurve{ f };
  // GameWindow<std::any>::DRAR_FUNCTION draw_curve = [&window, &bezierCurve,
  // &control_points, &ctx]() {
  //    std::shared_lock lock(ctx.vec_mutex);
  //    bezierCurve.drawCurve(sample_rate, control_points,
  ...
  GameWindow<std::any>::DRAR_FUNCTION draw_control_point = [&ctx, &window]() {
    ...
    for (int i = 0; i < ctx.control_points->size(); i++) {
      ...
      GAlgo::renderCircle((*ctx.control_points)[i], control_point_render_radio,
                          std::bind(&FrameBuffer::set_pixel, &window,
                                    std::placeholders::_1,
                                    std::placeholders::_2),
                          [](const RGB &whatever) {
  ...
  GameWindow<std::any>::DRAR_FUNCTIONS drawFunctions{draw_control_point,
                                                     draw_curve};
