...
struct CameraMachineContext {
  ...
  CameraMachineContext(int radio,
                       std::vector<POINT_EGDE_2D> *contorl_points = nullptr)
  ...
  boost::statechart::result react(const GComponent::MouseKeyboardEvent &event) {
    ...
    if (event.id == cv::EVENT_LBUTTONDOWN) {
      ...
      auto iterator = std::find_if(
          ctx.control_points->begin(), ctx.control_points->end(),
...
struct ModifyingControlPoints
    : public boost::statechart::state<ModifyingControlPoints, Machine> {
  ...
  boost::statechart::result react(const GComponent::MouseKeyboardEvent &event) {
    ...
    if (event.id == cv::EVENT_MOUSEMOVE) {
      *ctx.cached_moving_point =
          POINT_EGDE_2D{(float)event.x, (float)event.y, 1};
      return discard_event();
...
struct CreatingOrDoNothing
    : public boost::statechart::state<CreatingOrDoNothing, Machine> {
