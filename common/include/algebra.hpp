 ...
 */
Eigen::Matrix4f generateOrthogonalBasis(const Eigen::Vector4f &vector,
                                        const Eigen::Vector4f &point,
                                        int vector_offset);

/*
 * generate an matrix P that transform the xy coordinate to the new coordinate
 * system. the given vector is the nth-axis of the new basis.
 */
Eigen::Matrix4f generateBasisTransformationMatrix(const Eigen::Vector4f &vector,
                                                  const Eigen::Vector4f &point,
                                                  int vector_offset);
template <typename T>
...
template <typename T> T normalizeToLength(const T &vector, float length) {
  return vector.normalized() * length;
}

// normalize the vector to the given length inplace.
template <typename T> void normalizeToLengthInPlace(T &vector, float length) {
  vector.normalize();
...
bool insideCircle(const T &circle, const T &point, float radiu) {
  static_assert(g_type_traits::has_two_dimensions_v<T>,
                "using at least two dimensions vectors");
  return (std::pow(circle.x() - point.x(), 2) +
          std::pow(circle.y() - point.y(), 2)) < radiu;
}
