...
/*
 * std::declval<T>() is a prvalue of type T, used to simulate the creation of an
 * object of type T without the need to construct it. It is an hole in the type
 * system. decltype keyword can get the type of an expression. so
 * decltype(std::declval<std::ostringstream&>() << std::declval<T>(), void()) is
 * splited the folowing statement:
 * 1. std::declval<std::ostringstream&>() << std::declval<T>() equals expression
 * ` a << b`, where var a has type std::ostringstream, var b has type T
 * 2. `std::declval<std::ostringstream&>() << std::declval<T>(), void()` ends
 * with type void. Without void(), when T has multiple overrided << operator,
 * the compiler won't know which to choose, then throw an error.
 * 3. When T does not support std::ostream << T, substitution will fail,
 * has_ostream_operator<T> is subclass of false_type. When T support
 * std::ostream << T, substitution will success, has_ostream_operator<T> is
 * subclass of true_tpye. This is called SFINAE.
 *
 */
template <typename T>
struct has_ostream_operator<T, decltype(std::declval<std::ostringstream &>()
                                            << std::declval<T>(),
                                        void())> : std::true_type {};
...
template <typename T>
struct has_subscript_operator<T, decltype(std::declval<T>()[0], void())>
    : std::true_type {};
template <typename T>
inline constexpr bool has_subscript_operator_v =
    has_subscript_operator<T>::value;
// 获取元素类型
template <typename T> using element_type_t = decltype(std::declval<T>()[0]);

// 用于检查加法运算
template <typename T, typename = void> struct has_addition : std::false_type {};

template <typename T>
struct has_addition<T, decltype(std::declval<T>() + std::declval<T>(), void())>
    : std::true_type {};
...
template <typename T>
struct has_subtraction<T, decltype(std::declval<T>() - std::declval<T>(),
                                   void())> : std::true_type {};
...
template <typename T>
struct has_multiplication<T, decltype(std::declval<T>() * std::declval<T>(),
                                      void())> : std::true_type {};
...
template <typename T>
struct has_division<T, decltype(std::declval<T>() / std::declval<T>(), void())>
    : std::true_type {};

// 整合所有运算的类型萃取
template <typename T> struct has_arithmetic_operations {
...
template <typename T>
inline constexpr bool has_arithmetic_operations_v =
...
template <typename T>
struct has_two_dimensions<T, decltype((std::declval<T>().x(),
                                       std::declval<T>().y(), void()))>
...
template <typename T>
struct has_norm_functions<
    T, decltype(((std::declval<T>() - std::declval<T>()).norm()), void())>
    : public std::true_type {};
template <typename T>
inline constexpr bool has_norm_function = has_norm_functions<T>::value;

// trait eigen matrix's dimensions
template <typename T> struct eigen_matrix_dimension_trait {
