 ...
 */
template <typename T> bool insideTriangle(float x, float y, const T &edges) {
  static_assert(g_type_traits::has_subscript_operator_v<T>,
                "T should be support [] operator");
  ...
  for (int i = 0; i < 3; i++) {
    ...
    G_LOGGER_TRACE("ab(%.2f,%.2f) corss qa(%.2f,%.2f) equals %f", ab_x, ab_y,
                   qa_x, qa_y, cross_res);
    res += cross_res < 0 ? -1 : 1;
  }
...
std::tuple<float, float, float> computeBarycentric2D(float x, float y,
  ...
  // static_assert(compile_type_check, "v must be an array of 3 Eigen::Vector3f
  // or it's pointer");

  // float c1 = (x * (v[1].y() - v[2].y()) + (v[2].x() - v[1].x()) * y +
  // v[1].x() * v[2].y() - v[2].x() * v[1].y()) / (v[0].x() * (v[1].y() -
  // v[2].y()) + (v[2].x() - v[1].x()) * v[0].y() + v[1].x() * v[2].y() -
  // v[2].x() * v[1].y()); float c2 = (x * (v[2].y() - v[0].y()) + (v[0].x() -
  // v[2].x()) * y + v[2].x() * v[0].y() - v[0].x() * v[2].y()) / (v[1].x() *
  // (v[2].y() - v[0].y()) + (v[0].x() - v[2].x()) * v[1].y() + v[2].x() *
  // v[0].y() - v[0].x() * v[2].y()); float c3 = (x * (v[0].y() - v[1].y()) +
  // (v[1].x() - v[0].x()) * y + v[0].x() * v[1].y() - v[1].x() * v[0].y()) /
  // (v[2].x() * (v[0].y() - v[1].y()) + (v[1].x() - v[0].x()) * v[2].y() +
  // v[0].x() * v[1].y() - v[1].x() * v[0].y()); return { c1,c2,c3 };

  /*
   * the above ugly formual is the analytical solution of the following linear
   * question: If we use triangle vertices as the basis of a 2D plane, how to
   * compute coordiante of the same point. This coordinate system has a unique
   * name: Barycentric. more detail at tiger book 86.
   */

  // direct solution but not fast
  //   Matrix3f basis;
  //   for (int i = 0; i < 2; i++) {
  //	basis.col(i) = (v[i + 1] / v[i + 1].w() - v[i] /
  //v[i].w()).head<2>().homogeneous();
  //   }
...
std::tuple<float, float, float> computeBarycentric(const T &originCoor,
                                                   const U &v) {
  using coor_demension_trait = g_type_traits::eigen_matrix_dimension_trait<T>;
  using tri_demension_trait = g_type_traits::eigen_matrix_dimension_trait<
      g_type_traits::element_type_t<U>>;
  ...
  using EQUALTIONS =
      Eigen::Matrix<coor_demension_trait::scalar, dimonsion_size, 3>;
  EQUALTIONS equations =
      interpolation_internel::makeMatrix<coor_demension_trait::scalar,
                                         dimonsion_size, 3>();
  ...
  Eigen::Vector2f res =
      a.colPivHouseholderQr().solve(new_basis_coor.head(dimonsion_size));
