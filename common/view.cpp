...
Eigen::Matrix4f
view::generateOrthogonalProjection(const Eigen::Vector4f &down_left,
  ...
  return transformation::generateWindowingMatrix(
      down_left, top_right, Vector4f(-1, -1, 1, 1), Vector4f(1, 1, -1, 1));
}

Eigen::Matrix4f
view::generatePerspectiveProjection(const Eigen::Vector4f &down_left,
                                    const Eigen::Vector4f &top_right,
                                    float far) {
  float n = down_left.z();
  ...
  G_LOGGER_TRACE("new_top_right :\n%s",
                 EigenStructToString(new_top_right).c_str());
