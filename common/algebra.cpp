...
Eigen::Matrix4f algebra::generateOrthogonalBasis(const Eigen::Vector4f &vector,
                                                 const Eigen::Vector4f &point,
                                                 int vector_offset) {
  Matrix4f basis = Matrix4f::Zero();
  Vector4f normed_v = vector.normalized();
  Vector3f v = homogeneousToNormalCoordinate(normed_v);
  G_ASSERTS_TRUE(vector_offset >= 0 && vector_offset <= 2,
                 "wrong vector offset");
  ...
  basis.col(next) =
      Eigen::Map<Vector3f>(basis.col(vector_offset).head<3>().data())
          .cross(Eigen::Map<Vector3f>(
              basis.col((vector_offset + 1) % 3).head<3>().data()))
          .normalized()
          .homogeneous();
  basis.col(next)[3] = 0;
...
Eigen::Matrix4f
algebra::generateBasisTransformationMatrix(const Eigen::Vector4f &vector,
                                           const Eigen::Vector4f &point,
                                           int vectorOffset) {
